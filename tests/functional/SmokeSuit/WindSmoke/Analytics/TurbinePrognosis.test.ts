...
test("@Smoke Scenario 1 automation", async ({ loginPage, homePage, Analytics, SmokeCommonMethods }) => {
    ...
    await test.step('Enter Turbine Prognosis page', async () => {
        const isPagePresent: boolean = await homePage.NaviagetToMenu("Analytics", "Turbine Prognosis");
        if (isPagePresent) {
            ...
            await Analytics.IfDataIsVisible1();
            // await Analytics.CheckGraphvalue();
