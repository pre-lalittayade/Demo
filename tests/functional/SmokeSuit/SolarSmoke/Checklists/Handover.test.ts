...
test("@Smoke Scenario 1 automation", async ({ loginPage, homePage, Analytics, SmokeCommonMethods }) => {
    ...
    await test.step('Enter Handover page', async () => {
        const isPagePresent: boolean = await homePage.NaviagetToMenu("Check Lists", "Handover");
        if (isPagePresent) {
            ...
            await test.step('test validation', async () => {
                ...
                // console.log(columnValues[4]);
                const validationvalue = columnValues[4];
