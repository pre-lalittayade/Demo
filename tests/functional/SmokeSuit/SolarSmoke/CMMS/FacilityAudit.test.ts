...
test("@Smoke Scenario 1 automation", async ({ loginPage, homePage, CMMS_Master,SmokeCommonMethods }) => {
    ...
    //Activities//
    await test.step('Enter Facility Audit Check page', async () => {
        const isPagePresent: boolean = await homePage.NaviagetToMenu("CMMS", "Facility Audit");
        if (isPagePresent) {
            // await homePage.NaviagetToMenu(2, "Activities");
            // const responseTime = await CMMS_Master.GetPageResponseTime();
            await delay(7000);
           // await CMMS_Master.WFEA();
