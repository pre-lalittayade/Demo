...
test("@Smoke Scenario 1 automation", async ({ loginPage, homePage, Monitoring,SmokeCommonMethods }) => {
    ...
    await test.step('Entering to Block Page', async () => {
        ...
        await homePage.NaviagetToMenu("Monitoring", "Block");
        await delay(2000);
        ...
        await test.step('test validation', async () => {
            ...
            await Monitoring.CheckContractorcolumn();
            ...
            await Monitoring.ScrollToLeft();
            const columnValues: Array<string> = await Monitoring.GetHiddenColumnValuesEOSE(columns,3);
            // console.log(columns);
            // console.log(columnValues);
