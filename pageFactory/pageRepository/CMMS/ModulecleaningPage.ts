import { ModulecleaningPageObjects } from '@objects/CMMS/ModulecleaningPageObject';
...
export class ModulecleaningPage extends ModulecleaningPageObjects{
    readonly page: Page;
    ...
    await webactions.clickElement(ok_btn);
       ...
       async ClickOnUserSearchBtn(name:string){
        await webactions.clickElement(this.searchbuttons(name));
       }
       async ClickOnSearchBtn(name:string,value:string){
        await webactions.clickElement(this.searchbuttons(name));
       ...
       async AddCyclePlanDetails(name:string,code:string){
        await this.page.locator(this.AddData).scrollIntoViewIfNeeded();
        ...
        await webactions.delay(1000);
        N=await this.page.innerText(this.NOofModuleAssetMaster(code));
    ...
    async Addplanned_date(value:string,date:string){
        await webactions.clickElement(this.Planned_Date(value));
        await webactions.delay(1000);
...
async CheckModuleCleaningCycleName(chk:string){
    const checkmodulecleaning=await this.page.locator('//div[@class="ag-body-viewport ag-layout-normal ag-row-no-animation"]//div[@class="ag-center-cols-viewport"]//div[@row-index="0"]')
    await expect.soft(checkmodulecleaning).toContainText(chk);
}
async CheckModuleCleaningCycleApprover(chk:string){
    const checkmodulecleaning=await this.page.locator('//div[@class="ag-body-viewport ag-layout-normal ag-row-no-animation"]//div[@class="ag-center-cols-viewport"]//div[@row-index="0"]')
    await expect.soft(checkmodulecleaning).toContainText(chk);
}
async CheckModuleCleaningCycleAsset(value:string,chk:string){
...
async CheckNoofTables(name:string){
    let T=this.page.locator(this.NoofModuleAppeared(name));
    await webactions.delay(2000);
...
async ClickonCancel(){
  ...
  async Addremarks(remarks:string,value:string){
    await webactions.waitForElementAttached(this.Taskremarks(value));
    await webactions.enterElementText(this.Taskremarks(value),remarks);
    await webactions.delay(2000);
  }
}